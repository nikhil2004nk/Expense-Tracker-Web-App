      ...
      >
        ...
        <nav className="p-4 space-y-2">
          {navItems.map(({ to, label, icon }) => (
            ...
            >
              {({ isActive }) => (
                <div className={`flex items-center gap-3 rounded-lg px-3 py-3 text-sm font-medium transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2 dark:focus:ring-offset-gray-800 ${
                  isActive 
                  ...
                  <span className={`flex-shrink-0 ${isActive ? 'text-emerald-600 dark:text-emerald-400' : 'text-gray-400 dark:text-gray-500'}`}>
                    {icon}
                  </span>
                  <span>{label}</span>
                </div>
